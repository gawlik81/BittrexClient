...
public class Bittrex {
  ...
  public void setAuthKeysFromTextFile(String textFile) { // Add the text file containing the key & secret in the same path as the source code
    ...
    try (Scanner scan = new Scanner(getClass().getResourceAsStream(textFile))) {
      ...
      log.info(apikeyLine + " " + secretLine);
  ...
  public String withdraw(String currency, String quantity, String address,
      String paymentId) { // Withdraw a certain amount of a specific coin to an address, and add a payment id
  ...
  private HashMap<String, String> returnCorrectMap(
      String... parameters) { // Handles the exception of the generateHashMapFromStringList() method gracefully as to not have an excess of try-catch statements
  ...
  private HashMap<String, String> generateHashMapFromStringList(String... strings)
      throws Exception { // Method to easily create a HashMap from a list of Strings
  ...
  private static HashMap<String, String> jsonMapToHashMap(String jsonMap) {
    ...
    return gson.fromJson(jsonMap, new TypeToken<HashMap<String, String>>() {
    }.getType());
  }
  ...
  private String getResponseBody(final String baseUrl) {
    ...
    try {
      ...
      if (retryAttemptsLeft-- > 0) {
        ...
        System.err
            .printf("Could not connect to host - retrying in %d seconds... [%d/%d]%n", retryDelaySeconds, retryAttempts - retryAttemptsLeft, retryAttempts);
